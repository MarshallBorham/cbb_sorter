...
function ensureRadarFonts(): void {
...
function getStat(obj: PlayerLike, field: string, key: string): number | undefined {
  if (!obj) return undefined;
  const bag = obj[field];
  if (bag == null) return undefined;
  if (bag instanceof Map) return (bag as Map<string, number>).get(key);
  return (bag as Record<string, number>)[key];
}

function calcPercentiles(stat: string, pool: PlayerLike[], statsField: string): (val: number) => number {
...
function blendPercentile(statPcts: Record<string, number>, keys: string[]): number | null {
  const vals = keys.map((k) => statPcts[k]).filter((v): v is number => typeof v === "number" && !Number.isNaN(v));
...
async function getRadarStatPcts(player: PlayerLike, top100: boolean): Promise<Record<string, number> | null> {
  const statsField = top100 ? "statsTop100" : "stats";
  const query = top100
    ? { "statsTop100.G": { $exists: true, $gt: 0 }, "statsTop100.Min": { $gte: 15 } }
    : { "stats.Min": { $gte: 15 } };

  const pool = await Player.find(query).lean() as PlayerLike[];
...
export async function renderPlayerRadarPng(player: PlayerLike, top100: boolean): Promise<Buffer | null> {
  ensureRadarFonts();
